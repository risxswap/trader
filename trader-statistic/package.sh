...
CONFIG_PROPERTIES="$ROOT_DIR/src/main/resources/config.properties"
for required_file in "$APPLICATION_YML" "$LOGBACK_XML" "$DOCKER_COMPOSE" "$CONFIG_PROPERTIES"; do
...
cp -f "$JAR_PATH" "${PKG_DIR}/trader-statistic.jar"
cp -f "$DOCKER_COMPOSE" "${PKG_DIR}/docker-compose.yml"
cp -f "$APPLICATION_YML" "${PKG_DIR}/config/application.yml"
cp -f "$LOGBACK_XML" "${PKG_DIR}/config/logback-spring.xml"
cp -f "$CONFIG_PROPERTIES" "${PKG_DIR}/config/config.properties"
if [[ -d "$ROOT_DIR/bin" ]]; then
  cp -R "$ROOT_DIR/bin" "${PKG_DIR}/bin"
  chmod +x "${PKG_DIR}/bin/"*.sh 2>/dev/null || true
fi
if ! jar tf "${PKG_DIR}/trader-statistic.jar" >/dev/null 2>&1; then
  echo "Copied jar is invalid: ${PKG_DIR}/trader-statistic.jar"
  exit 1
fi
...
echo "Jar: ${PKG_DIR}/trader-statistic.jar"
ls -lh "${PKG_DIR}/trader-statistic.jar"
if command -v shasum >/dev/null 2>&1; then
  shasum -a 256 "${PKG_DIR}/trader-statistic.jar"
elif command -v sha256sum >/dev/null 2>&1; then
  sha256sum "${PKG_DIR}/trader-statistic.jar"
fi
