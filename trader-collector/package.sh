...
cp -f "$JAR_PATH" "${PKG_DIR}/trader-collector.jar"
...
if ! jar tf "${PKG_DIR}/trader-collector.jar" >/dev/null 2>&1; then
  echo "Copied jar is invalid: ${PKG_DIR}/trader-collector.jar"
  exit 1
fi
...
echo "Jar: ${PKG_DIR}/trader-collector.jar"
ls -lh "${PKG_DIR}/trader-collector.jar"
if command -v shasum >/dev/null 2>&1; then
  shasum -a 256 "${PKG_DIR}/trader-collector.jar"
elif command -v sha256sum >/dev/null 2>&1; then
  sha256sum "${PKG_DIR}/trader-collector.jar"
fi
