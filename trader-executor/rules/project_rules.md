...
4、Dao层负责数据库操作，封装数据库操作语句，使用MyBatis-Plus框架，数据库为MySQL + ClickHouse，缓存/消息为Redis，表结构设计符合数据库 normalization 范式。数据库表字段命名采用下划线命名法，主键字段命名为id，外键字段命名为${表名}_id，索引字段命名为${表名}_${字段名}_idx。
