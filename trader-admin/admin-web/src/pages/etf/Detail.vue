      ...
      <div class="page-header__content">
        <div class="page-title-row">
          ...
          <div>
            ...
            <div class="subtitle">统一查看基金基础档案、交易区间和 K 线走势</div>
          </div>
        </div>
    ...
    <el-card class="search-card">
      <div class="search-header">
        <div>
          ...
          <div class="section-subtitle">按基金代码与交易区间加载基础信息和 K 线数据</div>
        </div>
      ...
      <el-card class="chart-card">
        <div class="card-toolbar">
          <div>
            ...
            <div class="section-subtitle">按当前查询区间展示基金日线走势，缩放到边缘时自动加载更多数据</div>
          </div>
        ...
        <div v-else class="chart-empty">
          <el-empty description="当前区间暂无 K 线数据" :image-size="60" />
        </div>
      </el-card>
    </div>
  </div>
...
import { getFundDetail, listFunds, getFundMarket, type FundMarketDto, type FundDto, type PageRes } from '../../services/fund'
...
use([CanvasRenderer, CandlestickChart, GridComponent, TooltipComponent, DataZoomComponent, LegendComponent, TitleComponent])
...
const searchFunds = async (kw: string) => {
  ...
  try {
    const res = await listFunds({ pageNo: 1, pageSize: 10, keyword: kw, market: 'E' })
...
const loadWithFilters = async () => {
  ...
  try {
    ...
    const mr = await getFundMarket(payload)
    if (mr.code === 200 && mr.data) {
      const data = mr.data as FundMarketDto[]
      // Sort by time ascending for chart
      data.sort((a, b) => new Date(a.time).getTime() - new Date(b.time).getTime())
      
      const dates = data.map(d => d.time.substring(0, 10))
      // [open, close, low, high]
      ...
      loadedStart.value = payload.startDate
      loadedEnd.value = payload.endDate
    } else {
...
const loadMoreLeft = async () => {
  ...
  try {
    ...
    const mr = await getFundMarket(payload)
    if (mr.code === 200 && mr.data) {
      const data = (mr.data as FundMarketDto[]).filter(Boolean)
      if (data.length) {
        data.sort((a, b) => new Date(a.time).getTime() - new Date(b.time).getTime())
        
        const existing = new Set<string>(option.value.xAxis.data as string[])
        const prepend = data.filter(d => !existing.has(d.time.substring(0, 10)))
        
        if (prepend.length) {
          const dates = prepend.map(d => d.time.substring(0, 10))
          // [open, close, low, high]
          const values = prepend.map(d => [d.open, d.close, d.low, d.high])
          
          option.value.xAxis.data = [...dates, ...(option.value.xAxis.data as string[])]
          option.value.series[0].data = [...values, ...(option.value.series[0].data as any[])]
  ...
  try {
    ...
    if (mr.code === 200 && mr.data) {
      ...
      if (data.length) {
        ...
        const existing = new Set<string>(option.value.xAxis.data as string[])
        const append = data.filter(d => !existing.has(d.time.substring(0, 10)))
        
        if (append.length) {
          const dates = append.map(d => d.time.substring(0, 10))
          // [open, close, low, high]
          const values = append.map(d => [d.open, d.close, d.low, d.high])
          
          option.value.xAxis.data = [...(option.value.xAxis.data as string[]), ...dates]
          option.value.series[0].data = [...(option.value.series[0].data as any[]), ...values]
