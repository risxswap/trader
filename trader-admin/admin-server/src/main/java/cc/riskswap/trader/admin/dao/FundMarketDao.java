...
import cc.riskswap.trader.admin.dao.mapper.FundMarketMapper;
import cc.riskswap.trader.admin.dao.query.FundMarketListQuery;
...
public class FundMarketDao extends ServiceImpl<FundMarketMapper, FundMarket> {
    ...
    /**
     * Get fund market data by time and symbol
    ...
    public Page<FundMarket> pageQuery(FundMarketListQuery q) {
        ...
        if (q.getEndDate() != null) {
            wrapper.lt(FundMarket::getTime, q.getEndDate().plusDays(1).atStartOfDay(ZoneId.systemDefault()).toOffsetDateTime());
        }
        wrapper.orderByDesc(FundMarket::getTime);
